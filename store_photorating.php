...
SessionManager::sessionStart("vp", 0, "/~rinde/vp2021/Ryhm-1/", "greeny.cs.tlu.ee");
// kas on sisselogitud
if (isset($_SESSION["user_id"])) {
    ...
    if (!empty($id)) {
        ...
        if (!empty($rating)) {
            $stmt = $conn->prepare(
                "INSERT INTO vpr_photoratings (photoid, userid, rating) VALUES(?, ?, ?)"
            );
            echo $conn->error;
        ...
        $stmt = $conn->prepare(
            "SELECT AVG(rating) as avgValue FROM vpr_photoratings WHERE photoid = ?"
        );
        echo $conn->error;
