...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST["user_data_submit"])) {
        ...
        if (
            empty($birth_day_error) and
            empty($birth_month_error) and
            empty($birth_year_error)
        ) {
        ...
        if (
            empty($firstname_error) and
            empty($surname_error) and
            empty($birth_month_error) and
            empty($birth_year_error) and
            empty($birth_day_error) and
            empty($birth_date_error) and
            empty($gender_error) and
            empty($email_error) and
            empty($password_error) and
...
<body>
	...
	<form action="<?= htmlspecialchars($_SERVER["PHP_SELF"]) ?>" method="post">
		...
		<input id="first_name_input" name="first_name_input" type="text" value="<?= $firstname ?>"><span><?= $firstname_error ?></span><br>
		...
		<input id="gender_input_1" name="gender_input" type="radio" value="2" <?php if ($gender == "2") {echo " checked";} ?>>
		<label for="gender_input_1">Naine</label>
		<input id="gender_input_2" name="gender_input" type="radio" value="1" <?php if ($gender == "1") {echo " checked";} ?>>
		<label for="gender_input_2">Mees</label><br>
		<span><?= $gender_error ?></span><br>
		...
		echo "</select> \n";
		?>
		<span><?= $birth_date_error . " " . $birth_day_error . " " . $birth_month_error . " " . $birth_year_error ?></span><br>
		<label for="email_input">E-mail (kasutajatunnus):</label><br>
		<input id="email_input" name="email_input" type="email" value="<?= $email ?>"><span><?= $email_error ?></span><br>
		<label for="password_input">Salasõna (min 8 tähemärki):</label><br>
		<input id="password_input" name="password_input" type="password"><span><?= $password_error ?></span><br>
		<label for="confirm_password_input">Korrake salasõna:</label><br>
		<input id="confirm_password_input" name="confirm_password_input" type="password"><span><?= $confirm_password_error ?></span><br>
		<input name="user_data_submit" type="submit" value="Loo kasutaja"><span><?= $notice ?></span>
	</form>
	<hr>
	<p>Tagasi <a href="page2.php">avalehele</a></p>
</body>
</html>