...
function resize_photo($src, $w, $h, $keep_orig_proportion = true) {
    ...
    if ($w == $h) {
        ...
        // kui tuleb originaaproportsioone säilitada
        ...
        // kui on vaja kindlasti etteantud suurust, ehk pisut ka kärpida
        if ($image_w / $w < $image_h / $h) {
...
function store_photo_data($image_file_name, $alt, $privacy) {
    ...
    $stmt = $conn->prepare(
        "INSERT INTO vpr_photos (userid, filename, alttext, privacy) VALUES (?, ?, ?, ?)"
    );
    echo $conn->error;
    $stmt->bind_param(
        "issi",
